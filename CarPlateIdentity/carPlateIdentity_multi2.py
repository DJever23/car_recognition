...
def horizontal_cut_chars(plate):#传入车牌二值图像中的字符部分
    ...
    print('char_addr_list2',char_addr_list)#ex.char_addr_list=[(4, 20, 10), (20, 45, 14), (45, 62, 7), (62, 83, 13), (83, 96, 6), (96, 114, 14), (114, 132, 14)]
...
def get_chars(car_plate):#传入车牌二值化图像
    ...
    for i,addr in enumerate(char_addr_list):
        char_img = car_plate[chars_top:chars_bottom+1,addr[0]:addr[1]]#输出单个字符
...
def extract_char(car_plate):#传入正确的车牌
    gray_plate = cv2.cvtColor(car_plate,cv2.COLOR_BGR2GRAY)#转换成灰度图
    ...
    cv2.imwrite('./carIdentityData/opencv_output/extract_char_binary_plate.jpg',binary_plate)
    ...
    with sess1.as_default():
        with sess1.graph.as_default():#使用此图作为默认图的上下文管理器
            ...
            if result_index == -1:
                ...
                return True,result_plate_list#返回正确的索引对应的车牌
...
if __name__ == '__main__':
    ...
    # 字符提取
        char_img_list = extract_char(car_plate[i])
